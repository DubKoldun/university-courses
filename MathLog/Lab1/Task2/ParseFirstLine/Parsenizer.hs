...
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parsing","Expr","Disj","Conj","Not","Term","VAR","IMPL","OR","AND","NOT","LEFTBR","RIGHTBR","%eof"]
        bit_start = st * 16
        bit_end = (st + 1) * 16
...
happyReduction_10 (HappyTerminal (Variable happy_var_1))
	 =  HappyAbsSyn8
		 (Var happy_var_1
	)
...
happyError' = HappyIdentity . (\(tokens, _) -> errorParse tokens)
parsing tks = happyRunIdentity happySomeParser where
